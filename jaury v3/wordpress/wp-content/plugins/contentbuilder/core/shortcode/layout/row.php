...
class IG_Row extends IG_Pb_Shortcode_Layout {
			...
			'Notab' => array(
                ...
                array(
                    ...
                    'options' => array(
                        ...
                        'stripe' => __( 'Stripes', plumtree ),
                        'underline' => __( 'Underline', plumtree )
                    ),
                ),
	...
	function element_shortcode( $atts = null, $content = null ) {
        ...
        if ( isset($atts['background']) && ($atts['background'] != 'none') ) $out = "<div class=' ".@( $atts['r_class'] != 'none' ? $atts['r_class'] : "" )."' $style ".( ( isset( $atts['paralax']) && $atts['paralax'] == 'yes' ) ? " data-stellar-background-ratio='0.5' " : "" )." >";
        elseif (isset($atts['r_class']) && ( $atts['r_class'] != 'none' ) ) $out = "<div class=' ".@( $atts['r_class'] != 'none' ? $atts['r_class'] : "" )."'>";
        ...
        if ( array_key_exists('width', $atts) &&  ( $atts['width'] != 'no_cont' ) ) {
            ...
            else $out .= $custom_script . "" . "<div $extra_id class='row $extra_class' >" . IG_Pb_Helper_Shortcode::remove_autop( $content ) . '</div>' . '';
