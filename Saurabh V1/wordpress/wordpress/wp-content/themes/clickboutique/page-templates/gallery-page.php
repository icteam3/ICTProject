...
function pt_gallery( $blank = NULL, $attr ) {
    ...
    $gallery_div = "<div id='$selector' data-isotope='container' data-layout='$isotope_layout' class='pt-gallery gallery galleryid-{$id} gallery-columns-{$columns} gallery-size-{$size_class} {$isotope_layout}'>";
    ...
    foreach ( $attachments as $id => $attachment ) {
        ...
        if ( $captiontag && trim($attachment->post_title) ) {
            $output .= "
                <{$captiontag} class='gallery-item-description' data-mfp-src='".$image_src."'>
                <div class='desc-wrapper'><h3>" . wptexturize($attachment->post_title) . "</h3>";
                if ( !empty($attachment->post_excerpt) ) {
                    $output .= "<div>" . wptexturize($attachment->post_excerpt) . "</div>";
                }
                
            $output .= "</div><div class='vertical-helper'></div><i class='fa fa-search'></i></{$captiontag}>";
        }
        $output .= "</{$itemtag}>";
